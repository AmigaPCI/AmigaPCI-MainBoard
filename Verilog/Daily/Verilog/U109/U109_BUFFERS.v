...
(
    ...
    //PCI Signals
    input PCI_TIPn, BRIDGE_SPACE, CACHE_SPACE, BUFFER_EN, P2A_TIMEOUT, W_LATCH_FRAMEn, RETRY_CYCLE, //DEVSELn, 
...
always @(negedge CLK40) begin
    if (!RESETn) begin
        ...
        CACHE_SPACE_EN <= 0;
        A_LATCH <= 32'h0;
        BUFFER_EN_SYNC <= 2'b11;
    end else begin
        BUFFER_EN_SYNC <= {BUFFER_EN_SYNC[0], BUFFER_EN};        
        if (CPU_ADDRESS_VALID) begin
            if (BUFFER_EN_SYNC[1]) begin
                CPU_ADDRESS_VALID <= 0;
            end
        end else begin
...
wire AD_OUT_EN = ADDRESS_VALID || (BUFFER_EN && (CPU_BUS ^ RnW));
...
wire D_OUT_EN = (BUFFER_EN && (CPU_BUS == RnW));
...
assign PCI_BUF_DIR = ((!CPU_BUS && !BUFFER_EN) || (BUFFER_EN && (CPU_BUS == RnW)));
